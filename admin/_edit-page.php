<?php 
	...
	if (!isset($_GET['page_id']) || isset($_POST['cancel'])) {
	...
	$stmt = $conn->stmt_init();
		//prepare sql query
		$sql = 'SELECT page_id, image_id, cat_id, title, page_content, html_content
				FROM php_pages WHERE page_id = ?';
			if ($stmt->prepare($sql)) {
				// bind parameters
				$stmt->bind_param('i', $_GET['page_id']);
				// bind the results to variables
				$stmt->bind_result($page_id, $image_id, $cat_id, $page_title, $page_content, $html_content);
		...
		if (isset($_POST['insert']) && ($error === '') ) {
			...
			if (!isset($imageError)) {
				...
				if (isset($image_id) && isset($_POST['html_content'])) {
					...
					$sql = 'UPDATE php_pages SET image_id = ?, cat_id = ?, title = ?, page_content = ?, html_content = ?
							WHERE page_id = ?';
					$stmt->prepare($sql);
					$stmt->bind_param('iisssi', $image_id, $cat_id, $_POST['page_title'], $_POST['page_content'], $_POST['html_content'], $_POST['page_id']);
				} else if (isset($image_id) && !isset($_POST['html_content'])) {
					$sql = 'UPDATE php_pages SET image_id = ?, cat_id = ?, title = ?, page_content = ?, html_content = NULL
						WHERE page_id = ?';
					$stmt->prepare($sql);
					$stmt->bind_param('iissi', $image_id, $cat_id, $_POST['page_title'], $_POST['page_content'], $_POST['page_id']);
				} else if (!isset($image_id) && isset($_POST['html_content'])) {
					$sql = 'UPDATE php_pages SET image_id = NULL, cat_id = ?, title = ?, page_content = ?, html_content = ?
							WHERE page_id = ?';
					$stmt->prepare($sql);
					$stmt->bind_param('isssi', $cat_id, $_POST['page_title'], $_POST['page_content'], $_POST['html_content'], $_POST['page_id']);
				} else {
					//create sql
					$sql = 'UPDATE php_pages SET image_id = NULL, cat_id = ?, title = ?, page_content = ?, html_content = NULL
							WHERE page_id = ?';
					$stmt->prepare($sql);
					$stmt->bind_param('issi', $cat_id, $_POST['page_title'], $_POST['page_content'], $_POST['page_id']);
				}
	...
	$title_section = "Admin - Update Page";
						...
						<fieldset>
							...
							<ol>
								<li <?php if(isset($errorTitle)) echo 'class="error"'?>>
									...
									<input type="text" name="page_title" id="page_title" value="<?php if (isset($error)) {
										echo htmlentities($_POST['page_title'], ENT_COMPAT, 'utf-8'); 
										} else {
											echo htmlentities($page_title, ENT_COMPAT, 'utf-8');
										}
										?>">
								</li>
								<li <?php if(isset($errorPage)) echo 'class="error"'?>>
									<label for="page_content">Page Content:</label>
									<textarea name="page_content" cols="60" rows="24" id="page_content"><?php if (isset($error)) {
										echo htmlentities($_POST['page_content'], ENT_COMPAT, 'utf-8');
									} else {
										echo htmlentities($page_content, ENT_COMPAT, 'utf-8');
										} 
										?></textarea>
								</li>
								<li>
							...
							<li>
								...
								<input name="page_id" type="hidden" value="<?php if(isset($page_id)) { echo $page_id; } else { echo $_POST['page_id']; } ?>">
							</li>
						</ol>
					</fieldset>
				</form>
