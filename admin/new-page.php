	...
	if ((isset($_POST['insert'])) && ($error === '')) {
		...
		if (!isset($imageError)) {
			...
			if (isset($image_id) && isset($_POST['html_content'])) {
				$sql = 'INSERT INTO php_pages (image_id, cat_id, title, page_content, html_content, created)
				VALUES(?, ?, ?, ?, ?, NOW())';
				$stmt->prepare($sql);
				$stmt->bind_param('iisss', $image_id, $cat_id, $_POST['post_title'], $_POST['page_content'], $_POST['html_content']);
			} else if (isset($image_id) && !isset($_POST['html_content'])) {
				$sql = 'INSERT INTO php_pages (image_id, cat_id, title, page_content, created)
				VALUES(?, ?, ?, ?, NOW())';
				$stmt->prepare($sql);
				$stmt->bind_param('iiss', $image_id, $cat_id, $_POST['post_title'], $_POST['article']);
			} else if (!isset($image_id) && isset($_POST['html_content'])) {
				$sql = 'INSERT INTO php_pages (cat_id, title, page_content, html_content, created)
				VALUES(?, ?, ?, ?, NOW())';
				$stmt->prepare($sql);
				$stmt->bind_param('isss', $cat_id, $_POST['post_title'], $_POST['article'], $_POST['html_content']);
			} else {
				//create sql
				$sql = 'INSERT INTO php_pages (cat_id, title, page_content, created)
				VALUES(?, ?, ?, NOW())';
				$stmt->prepare($sql);
...
?>
			<article>
				...
				<p>Don't forget to create a web page to hold this new content!</p>
				...
				if (isset($error)) {
					...
					<form id="form2" method="post" enctype="multipart/form-data">
						<fieldset>
							<legend>Create New Page Content</legend>
							<ol>
